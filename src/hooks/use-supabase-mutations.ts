...
function normalize_gara_livello(value?: string) {
  ...
  const aliases: Record<string, string> = {
    pulcini: 'Pulcini',
    'stellina 1': 'Stellina 1', stellina1: 'Stellina 1',
    'stellina 2': 'Stellina 2', stellina2: 'Stellina 2',
    'stellina 3': 'Stellina 3', stellina3: 'Stellina 3',
    'stellina 4': 'Stellina 4', stellina4: 'Stellina 4',
    interbronzo: 'Interbronzo', bronzo: 'Bronzo',
    interargento: 'Interargento', argento: 'Argento',
    interoro: 'Interoro', oro: 'Oro',
  };
...
export function use_upsert_istruttore() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const payload = {
        club_id: DEMO_CLUB_ID, nome: data.nome, cognome: data.cognome,
...
export function use_upsert_corso() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const payload: any = {
        club_id: DEMO_CLUB_ID, nome: data.nome, tipo: data.tipo || '',
      ...
      if (data.istruttori_ids) {
        ...
        if (unique_ids.length > 0) {
          ...
          const { error: insert_error } = await supabase.from('corsi_istruttori').upsert(rows, { onConflict: 'corso_id,istruttore_id' });
...
export function use_upsert_gara() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const payload = {
        club_id: DEMO_CLUB_ID, nome: data.nome, data: data.data,
        ora: normalize_time(data.ora, '09:00:00'),
        club_ospitante: data.club_ospitante || '', indirizzo_club_ospitante: data.indirizzo_club_ospitante || '',
        localita: data.localita || '', livello_minimo: normalize_gara_livello(data.livello_minimo),
        carriera: normalize_gara_carriera(data.carriera),
        costo_iscrizione: data.costo_iscrizione || 0, costo_accompagnamento: data.costo_accompagnamento || 0,
        note: data.note || '',
      };
      if (data.id) {
  ...
  return useMutation({
    mutationFn: async (data: { atleta_id: string; gara_id: string; carriera?: string; punteggio?: number; voto_giudici?: number; posizione?: number; medaglia?: string; costo_iscrizione?: number; costo_accompagnamento?: number }) => {
      const { error } = await supabase.from('iscrizioni_gare').insert({
        atleta_id: data.atleta_id, gara_id: data.gara_id, carriera: data.carriera || '',
        punteggio: data.punteggio || null, voto_giudici: data.voto_giudici || null,
        posizione: data.posizione || null, medaglia: data.medaglia || null,
        costo_iscrizione: data.costo_iscrizione || 0, costo_accompagnamento: data.costo_accompagnamento || 0,
      });
      if (error) throw error;
    },
...
export function use_crea_lezione_privata() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const base_payload = {
        club_id: DEMO_CLUB_ID, istruttore_id: data.istruttore_id,
        ora_inizio: normalize_time(data.ora_inizio, '00:00:00'),
        ora_fine: normalize_time(data.ora_fine, '00:20:00'),
        durata_minuti: data.durata_minuti || 20, ricorrente: !!data.ricorrente,
        condivisa: (data.atleti_ids?.length || 0) > 1,
        costo_totale: data.costo_totale || 0, annullata: false, note: data.note || '',
      };
      if (data.ricorrente) {
        const { data: stagione, error: stagione_error } = await supabase
          .from('stagioni').select('id, data_fine')
          .eq('club_id', DEMO_CLUB_ID).eq('attiva', true).eq('tipo', 'Regolare')
          .order('data_fine', { ascending: true }).limit(1).maybeSingle();
      ...
      const { data: lezione, error } = await supabase.from('lezioni_private').insert({ ...base_payload, data: data.data }).select('id').single();
      if (error) throw error;
...
export function use_aggiungi_atleta_lezione() {
  ...
  return useMutation({
    mutationFn: async (data: {
      lezione_id: string; atleta_id: string; nuovo_costo_totale: number;
      modalita_costo: 'dividi' | 'manuale'; atleti_ids_esistenti: string[];
    }) => {
      const tutti_atleti = [...data.atleti_ids_esistenti, data.atleta_id];
      const quota = data.nuovo_costo_totale / tutti_atleti.length;
      const { error: err_update } = await supabase.from('lezioni_private')
      ...
      for (const aid of data.atleti_ids_esistenti) {
        const { error } = await supabase.from('lezioni_private_atlete')
          .update({ quota_costo: quota }).eq('lezione_id', data.lezione_id).eq('atleta_id', aid);
        if (error) throw error;
      }
      const { error: err_insert } = await supabase.from('lezioni_private_atlete')
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('fatture').update({
        pagata: true, data_pagamento: new Date().toISOString().split('T')[0],
      }).eq('id', id);
      if (error) throw error;
    },
    ...
    mutationFn: async () => {
      ...
      for (const isc of (iscrizioni || [])) {
        ...
        fatture_da_creare.push({
          club_id: DEMO_CLUB_ID, atleta_id: isc.atleta_id,
          numero: `F-${String(next_num++).padStart(4, '0')}`,
          descrizione: `Corso ${corso.nome} - ${now.toLocaleString('it-CH', { month: 'long', year: 'numeric' })}`,
          importo: corso.costo_mensile || 0, data_emissione: now.toISOString().split('T')[0],
          data_scadenza: scadenza, pagata: false, tipo: 'corso', riferimento_id: corso.id,
        });
      }
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const { error } = await supabase.from('comunicazioni').insert({
        club_id: DEMO_CLUB_ID, titolo: data.titolo, testo: data.testo,
        tipo_destinatari: data.tipo_destinatari || 'tutti', corso_id: data.corso_id || null,
      });
      if (error) throw error;
    },
...
export function use_upsert_stagione() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const payload = {
        club_id: DEMO_CLUB_ID, nome: data.nome, tipo: data.tipo || 'regolare',
        data_inizio: data.data_inizio, data_fine: data.data_fine, attiva: data.attiva !== false,
      };
      if (data.id) {
...
export function use_upsert_campo() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const payload = {
        club_id: DEMO_CLUB_ID, nome: data.nome, data_inizio: data.data_inizio, data_fine: data.data_fine,
        luogo: data.luogo || '', club_ospitante: data.club_ospitante || '',
        costo_diurno: data.costo_diurno || 0, costo_completo: data.costo_completo || 0, note: data.note || '',
      };
      if (data.id) {
  ...
  return useMutation({
    mutationFn: async (data: { campo_id: string; atleta_id: string; tipo: string; costo_totale: number }) => {
      const { error } = await supabase.from('iscrizioni_campo').insert({
        campo_id: data.campo_id, atleta_id: data.atleta_id, tipo: data.tipo, costo_totale: data.costo_totale,
      });
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['campi'] }),
  ...
  return useMutation({
    mutationFn: async (data: { istruttore_id: string; disponibilita: Record<string, { ora_inizio: string; ora_fine: string }[]> }) => {
      ...
      for (const [giorno, slots] of Object.entries(data.disponibilita)) {
        for (const s of slots) rows.push({ istruttore_id: data.istruttore_id, giorno, ora_inizio: s.ora_inizio, ora_fine: s.ora_fine });
      }
      if (rows.length > 0) {
