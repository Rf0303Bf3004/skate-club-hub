...
export function use_annulla_lezione() {
  ...
  return useMutation({
    mutationFn: async (lezione_id: string) => {
      const { error: err1 } = await supabase.from("lezioni_private_atlete").delete().eq("lezione_id", lezione_id);
      if (err1) throw err1;

      const { error: err2 } = await supabase.from("lezioni_private").delete().eq("id", lezione_id);
      if (err2) throw err2;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["lezioni_private"] }),
  });
