...
import { use_segna_fattura_pagata, use_genera_fatture_mensili } from '@/hooks/use-supabase-mutations';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Select value={status_filter} onValueChange={set_status_filter}>
        <SelectTrigger className="w-48"><SelectValue placeholder={t('stato')} /></SelectTrigger>
        <SelectContent>
            ...
            <tbody>
              {filtered.map((f: any) => (
                <tr key={f.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">
                    {f.stato !== 'pagata' && (
                      <Button size="sm" variant="outline" onClick={(e) => handle_paga(e, f.id)} disabled={segna_pagata.isPending}>
