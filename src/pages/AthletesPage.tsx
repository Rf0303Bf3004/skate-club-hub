...
const AthletesPage: React.FC = () => {
  ...
  const fields: FormField[] = [
    ...
    {
      key: "percorso_amatori",
      label: t("percorso_amatori"),
      type: "select",
      options: levels.filter((l) => l !== "tutti").map((l) => ({ value: l, label: t(l) })),
    },
              ...
              {filtered.map((a: any) => (
                <tr key={a.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button variant="ghost" size="sm" onClick={() => open_edit(a)} className="text-xs h-7">
                      Modifica
                    </Button>
                  </td>
                </tr>
