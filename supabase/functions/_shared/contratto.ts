// Contratto di adesione: unica costruzione lato server.
// Le clausole sono copiate da src/lib/contratto-adesione.ts senza modifiche:
// cambiarle qui cambierebbe il testo dei contratti futuri e romperebbe il
// confronto con quello già mostrato alla famiglia.
...
/** Testo lineare archiviato: è esattamente quello che la pagina mostra. */
export function testo_contratto(articoli: ArticoloContratto[]): string {
  return articoli.map((a) => `Art. ${a.numero} — ${a.titolo}\n${a.testo}`).join("\n\n");
}

/** Impronta del testo: serve solo a verificare che nulla sia cambiato. */
export async function impronta_contratto(testo: string): Promise<string> {
...
/** Costruisce articoli, testo e impronta in un colpo solo. */
export async function contratto_completo(dati: DatiContratto) {
  const articoli = build_contratto(dati);
  const testo = testo_contratto(articoli);
  const impronta = await impronta_contratto(testo);
  return { articoli, testo, impronta };
}
