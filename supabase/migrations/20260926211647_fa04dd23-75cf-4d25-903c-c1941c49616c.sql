...
AS $function$
  WITH atleta AS (
    SELECT id, club_id, categoria, livello_amatori, livello_artistica, livello_stile,
           CASE
             WHEN categoria = 'artistica' THEN NULL
             WHEN categoria IN ('pulcini', 'amatori') THEN COALESCE(livello_amatori, livello_attuale)
             ELSE livello_attuale
           END AS livello_confronto
  ...
  WHERE c.attivo = true
    AND (
      ...
      OR (c.percorso IS NULL AND EXISTS (
            ...
            WHERE btrim(v.liv) IN (
              CASE WHEN a.categoria = 'artistica' THEN NULL ELSE a.livello_confronto END,
              CASE WHEN a.categoria = 'artistica' THEN a.livello_artistica END,
              CASE WHEN a.categoria = 'artistica' THEN a.livello_stile END)))
